...
sub my_update_cfg ($) {
    ...
    store $cfg, "$cfgpath/mycfg$cfgname.data";
    store $probes, "$cfgpath/myprobes$cfgname.data";
}
...
sub my_load_cfg ($) {
    ...
    $cfgname =~ s/config//g;
    my $cfmod = (stat $cfgfile)[9] || die "ERROR: loading smokeping configuration file $cfgfile: $!\n";
    if (! -f "$cfgpath/mycfg$cfgname.data") {
        load_cfg $cfgfile;
        my_update_cfg $cfgfile;
    }
    use Storable;
    local $Storable::Eval = 1;
    $cfg = undef;
    $cfg = Storable::retrieve("$cfgpath/mycfg$cfgname.data") || die "ERROR: loading configuration data: $!\n";
    $probes = undef;
    $probes = Storable::retrieve("$cfgpath/myprobes$cfgname.data") || die "ERROR: loading probes data: $!\n";
    if ($cfg->{__last} < $cfmod) {
        load_cfg $cfgfile;
        my_update_cfg $cfgfile;
    }
